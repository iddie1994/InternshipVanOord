...
# Outcomes
outcomes =  [
             TimeSeriesOutcome('Net present value vs time')
             TimeSeriesOutcome('Cumulative hydrogen production'),
             TimeSeriesOutcome('Cumulative power to grid'),
             TimeSeriesOutcome('Cumulative direct costs')
             ]
...
uncertainties =     [
                    ...
                    RealParameter('Hydrogen price', 1.75, 2.50),
                    RealParameter('Subsidy price', 1, 2),
                    ...
                    RealParameter('Drift parameter', 1.099, 1.1),
                    RealParameter('Volatility parameter', 0.3, 0.4),
                    RealParameter('Noise seed', 2, 3)
                     ]
...
results = perform_experiments(model, scenarios)

save_results(results, './data/12Wind6H2_DoggerBank_AEL_Islandpower_sensitivity4.tar.gz')
